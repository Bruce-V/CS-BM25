...
mytopicdb=myclient["cs2017_11"]
mydata=mytopicdb["cs2017_score_11"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2017_score_11_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_ele_2 = log((13670358 - 1665 + 0.5) / (1665 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                bm25_gastric_score = (((k1 + 1) * gastric_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + gastric_score))
                ...
                bm25_ab_score = idf_gastric * bm25_gastric_score + idf_pik3ca * bm25_pik3ca_score + idf_e545k * bm25_e545k_score

                idf_para = [{str(gastric_score): idf_gastric}, {str(pik3ca_score): idf_pik3ca},{str(e545k_score): idf_e545k}]
                for ele in ChemicalNameList:
                    if 'E545K' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_1
                        break
                ...
                for ele in ChemicalNameList:
                    if 'Gastric Mucosa' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_4
                        break
                for eleM in MeshHeadingNameList:
                    if 'Stomach Neoplasms' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_1
                        break
                for eleM in MeshHeadingNameList:
                ...
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                    if 'Gastric' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_6
                        break
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
