...
mytopicdb=myclient["cs2017_20"]
mydata=mytopicdb["cs2017_score_20"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2017_score_20_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_mdm2 = log((29138919 - 5399 + 0.5) / (5399 + 0.5), 10)
    idf_amplification = log((29138919 - 110630 + 0.5) / (110630 + 0.5), 10)

    idf_ele_1 = log((13670358 - 3864 + 0.5) / (3864 + 0.5), 10)
    idf_ele_2 = log((13670358 - 0 + 0.5) / (0 + 0.5), 10)
    idf_ele_3 = log((13670358 - 0 + 0.5) / (0 + 0.5), 10)
    idf_ele_4 = log((13670358 - 5415 + 0.5) / (5415 + 0.5), 10)
    idf_ele_5 = log((13670358 - 0 + 0.5) / (0 + 0.5), 10)

    idf_eleM_1 = log((25389659 - 0 + 0.5) / (0 + 0.5), 10)
    idf_eleM_2 = log((25389659 - 4575 + 0.5) / (4575 + 0.5), 10)
    idf_eleM_3 = log((25389659 - 548 + 0.5) / (548 + 0.5), 10)
    idf_eleM_4 = log((25389659 - 5415 + 0.5) / (5415 + 0.5), 10)
    ...
    idf_eleM_8 = log((25389659 - 702001 + 0.5) / (702001 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
            ...
            # ---------------共现分析关键字-------------------#
                ...
                bm25_mdm2_score = (((k1 + 1) * mdm2_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + mdm2_score))
                bm25_amplification_score = (((k1 + 1) * amplification_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + amplification_score))

                bm25_ab_score = idf_liposarcoma * bm25_liposarcoma_score + idf_mdm2 * bm25_mdm2_score + idf_amplification * bm25_amplification_score

                idf_para = [{str(liposarcoma_score): idf_liposarcoma}, {str(mdm2_score): idf_mdm2},
                ...
                for ele in ChemicalNameList:
                    # if re.findall(r'(BRAF|Proto-Oncogene Proteins B-raf|human|humans|male)',ele['NameOfSubstance']):
                    if 'Liposarcoma' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_2
                        break
                for ele in ChemicalNameList:
                    # if re.findall(r'(BRAF|Proto-Oncogene Proteins B-raf|human|humans|male)',ele['NameOfSubstance']):
                    if 'Myxoid' in ele['NameOfSubstance']:
                        ss1 = ss1 +idf_ele_3
                        break
                for ele in ChemicalNameList:
                    # if re.findall(r'(BRAF|Proto-Oncogene Proteins B-raf|human|humans|male)',ele['NameOfSubstance']):
                ...
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                    if 'Myxoid' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_3
                        break
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
