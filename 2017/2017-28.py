...
mytopicdb=myclient["cs2017_28"]
mydata=mytopicdb["cs2017_score_28"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2017_score_28_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_ductal = log((29138919 - 36952 + 0.5) / (36952 + 0.5), 10)
    idf_erbb3 = log((29138919 - 1031 + 0.5) / (1031 + 0.5), 10)

    idf_ele_1 = log((13670358 - 391 + 0.5) / (391 + 0.5), 10)
    idf_ele_2 = log((13670358 - 1608 + 0.5) / (1608 + 0.5), 10)


    idf_eleM_1 = log((25389659 - 0 + 0.5) / (0 + 0.5), 10)
    idf_eleM_2 = log((25389659 - 0 + 0.5) / (0 + 0.5), 10)
    idf_eleM_3 = log((25389659 - 134198 + 0.5) / (134198 + 0.5), 10)
    idf_eleM_4 = log((25389659 - 0 + 0.5) / (0 + 0.5), 10)
    idf_eleM_5 = log((25389659 - 1608 + 0.5) / (1608 + 0.5), 10)
    idf_eleM_6 = log((25389659 - 12232 + 0.5) / (12232 + 0.5), 10)
    idf_eleM_7 = log((25389659 - 46290 + 0.5) / (46290 + 0.5), 10)
    ...
    idf_eleK_2 = log((5435471 - 95 + 0.5) / (95 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                adenocarcinoma = [True for x in wordfreq.items() if 'pancreatic' in x]
                erbb3 = [True for x in wordfreq.items() if 'erbb3' in x]
                # ---------------摘要统计-------------------#
                for key in wordfreq:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'ductal ' in key1:
                        ductal_score = ductal_score + wordfreq[key]
                for key in wordfreq:
                    key1 = cop.sub('', key)
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'erbb3' in key1:
                        erbb3_score = erbb3_score + wordfreq[key]

                #---------------共现分析摘要-------------------#
                if len(pancreatic) != 0 and pancreatic[0] and len(adenocarcinoma) != 0 and adenocarcinoma[0] and len(erbb3) != 0 and erbb3[0]:
                    for key in wordfreq:
                        key1 = cop.sub('', key)
                        if 'erbb3' in key1:
                            gx=idf_erbb3
                ...
                if len(pancreatic) != 0 and pancreatic[0] and len(adenocarcinoma) != 0 and adenocarcinoma[0] and len(
                        erbb3) != 0 and erbb3[0]:
                    for eleM in MeshHeadingNameList:
                        if 'ErbB-3' in eleM['MeshHeadingName']:
                            gx = idf_erbb3
                            break
                ...
                if len(pancreatic) != 0 and pancreatic[0] and len(adenocarcinoma) != 0 and adenocarcinoma[0] and len(
                    ...
                    for ele in ChemicalNameList:
                        if 'ErbB-3' in ele['NameOfSubstance']:
                            gx = idf_erbb3
                            break
                ...
                if len(pancreatic) != 0 and pancreatic[0] and len(adenocarcinoma) != 0 and adenocarcinoma[0] and len(
                    ...
                    for eleK in KeywordsList:
                        if re.findall(r'(erbb3|erbb-3)', str(eleK).lower()):
                            gx = idf_erbb3
                            break
                ...
                bm25_ductal_score = (((k1 + 1) * ductal_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + ductal_score))
                bm25_erbb3_score = (((k1 + 1) * erbb3_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + erbb3_score))

                bm25_ab_score = idf_pancreatic * bm25_pancreatic_score + idf_adenocarcinoma * bm25_adenocarcinoma_score + idf_ductal * bm25_ductal_score + idf_erbb3 * bm25_erbb3_score

                idf_para = [{str(pancreatic_score): idf_pancreatic}, {str(adenocarcinoma_score): idf_adenocarcinoma},
                            {str(ductal_score): idf_ductal}, {str(erbb3_score): idf_erbb3}]

                for ele in ChemicalNameList:
                    # if re.findall(r'(BRAF|Proto-Oncogene Proteins B-raf|human|humans|male)',ele['NameOfSubstance']):
                    if 'ERBB3' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_1
                        break
                for ele in ChemicalNameList:
                    # if re.findall(r'(BRAF|Proto-Oncogene Proteins B-raf|human|humans|male)',ele['NameOfSubstance']):
                    if 'Receptor, ErbB-3' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_2
                        break
                ...
                for eleM in MeshHeadingNameList:
                    ...
                    if 'Pancreatic ductal adenocarcinoma' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_1
                        break
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                ...
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                    if 'ERBB3' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_4
                        break
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                    if 'Receptor, ErbB-3' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_5
                        break
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                    if 'Biopsy, Fine-Needle' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_6
                        break
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                ...
                for eleK in KeywordsList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                    if 'Pancreatic ductal adenocarcinoma' in str(eleK).lower():
                        ss4 = ss4 + idf_eleK_1
                        break
                for eleK in KeywordsList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                    if 'erbb3' in str(eleK).lower():
                        ss4 = ss4 + idf_eleK_2
                        break
