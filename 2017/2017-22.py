...
mytopicdb=myclient["cs2017_22"]
mydata=mytopicdb["cs2017_score_22"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2017_score_22_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_ele_1 = log((13670358 - 8524 + 0.5) / (8524 + 0.5), 10)
    idf_ele_2 = log((13670358 - 22004 + 0.5) / (22004 + 0.5), 10)
    idf_ele_3 = log((13670358 - 24866 + 0.5) / (24866 + 0.5), 10)
    idf_ele_4 = log((13670358 - 0 + 0.5) / (0 + 0.5), 10)

    idf_eleM_1 = log((25389659 - 510147 + 0.5) / (510147 + 0.5), 10)
    idf_eleM_2 = log((25389659 - 16241 + 0.5) / (16241 + 0.5), 10)
    ...
    idf_eleM_6 = log((25389659 - 24866 + 0.5) / (24866 + 0.5), 10)
    ...
    idf_eleK_1 = log((5435471 - 67747 + 0.5) / (1468 + 0.5), 10)
    idf_eleK_2 = log((5435471 - 330 + 0.5) / (29226 + 0.5), 10)
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
            ...
            # ---------------共现分析关键字-------------------#
                ...
                bm25_erbb2_score = (((k1 + 1) * erbb2_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + erbb2_score))
                bm25_amplification_score = (((k1 + 1) * amplification_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + amplification_score))

                bm25_ab_score = idf_lung * bm25_lung_score + idf_erbb2 * bm25_erbb2_score + idf_amplification * bm25_amplification_score

                idf_para = [{str(lung_score): idf_lung}, {str(erbb2_score): idf_erbb2},{str(amplification_score): idf_amplification}]
                for ele in ChemicalNameList:
                ...
                for ele in ChemicalNameList:
                    # if re.findall(r'(BRAF|Proto-Oncogene Proteins B-raf|human|humans|male)',ele['NameOfSubstance']):
                    if 'Receptor, ErbB-2' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_2
                        break
                for ele in ChemicalNameList:
                    # if re.findall(r'(BRAF|Proto-Oncogene Proteins B-raf|human|humans|male)',ele['NameOfSubstance']):
                    if 'Intercellular Signaling Peptides and Proteins' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_3
                        break
                for ele in ChemicalNameList:
                    # if re.findall(r'(BRAF|Proto-Oncogene Proteins B-raf|human|humans|male)',ele['NameOfSubstance']):
                ...
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                    if 'Receptor, ErbB-2' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_3
                        break
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                    if 'ERBB2 protein, human' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_4
                        break
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                    if 'herstatin' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_5
                        break
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
                    if 'Intercellular Signaling Peptides and Proteins' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_6
                        break
                for eleM in MeshHeadingNameList:
                    # if re.findall(r'(Melanoma|Proto-Oncogene Proteins B-raf|Humans|Neoplasms|Neoplasm|Male|Mutation|Mutational)',eleM['MeshHeadingName']):
