...
mytopicdb=myclient["cs2017_18"]
mydata=mytopicdb["cs2017_score_18"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2017_score_18_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_ele_1 = log((13670358 - 596 + 0.5) / (596 + 0.5), 10)
    idf_ele_2 = log((13670358 - 1056 + 0.5) / (1056 + 0.5), 10)

    idf_eleM_1 = log((25389659 - 0 + 0.5) / (0 + 0.5), 10)
    idf_eleM_2 = log((25389659 - 67099 + 0.5) / (67099 + 0.5), 10)
    idf_eleM_3 = log((25389659 - 1056 + 0.5) / (1056 + 0.5), 10)
    idf_eleM_4 = log((25389659 - 16241 + 0.5) / (16241 + 0.5), 10)
    idf_eleM_5 = log((25389659 - 268832 + 0.5) / (268832 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                wordfreq = x['wordfreq']
                pancreatic = [True for x in wordfreq.items() if 'prostate' in x]
                # ---------------摘要统计-------------------#
                for key in wordfreq:
            ...
            # ---------------共现分析关键字-------------------#
                ...
                bm25_cdk6_score = (((k1 + 1) * cdk6_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + cdk6_score))
                bm25_amplification_score = (((k1 + 1) * amplification_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + amplification_score))

                bm25_ab_score = idf_pancreatic * bm25_pancreatic_score + idf_cdk6 * bm25_cdk6_score + idf_amplification * bm25_amplification_score

                idf_para = [{str(pancreatic_score): idf_pancreatic}, {str(cdk6_score): idf_cdk6},
                ...
                for ele in ChemicalNameList:
                    if 'CDK6' in ele['NameOfSubstance']:
                        ...
                        break
                for ele in ChemicalNameList:
                    if 'Cyclin-Dependent Kinase 6' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_2
                        break
                for eleM in MeshHeadingNameList:
                    if 'CDK6' in eleM['MeshHeadingName']:
                ...
                for eleM in MeshHeadingNameList:
                    if 'Pancreatic Neoplasms' in eleM['MeshHeadingName']:
                        ...
                        break
                for eleM in MeshHeadingNameList:
                    if 'Cyclin-Dependent Kinase 6' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_3
                        break
                for eleM in MeshHeadingNameList:
