...
mytopicdb=myclient["cs2017_8"]
mydata=mytopicdb["cs2017_score_8"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2017_score_8_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_eml4alk = log((29138919 - 536 + 0.5) / (536 + 0.5), 10)
    idf_transcript = log((29138919 - 75198 + 0.5) / (75198 + 0.5), 10)

    idf_ele_1 = log((13670358 - 367 + 0.5) / (367 + 0.5), 10)
    idf_ele_2 = log((13670358 - 0 + 0.5) / (0 + 0.5), 10)


    idf_eleM_1 = log((25389659 - 200349 + 0.5) / (200349 + 0.5), 10)
    idf_eleM_2 = log((25389659 - 46148 + 0.5) / (46148 + 0.5), 10)
    idf_eleM_3 = log((25389659 - 17437618 + 0.5) / (17437618 + 0.5), 10)
    idf_eleM_4 = log((25389659 - 8002162 + 0.5) / (8002162 + 0.5), 10)
    idf_eleM_5 = log((25389659 - 2842020 + 0.5) / (2842020 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},no_cursor_timeout=True):
        ss1 = 0
        ss2 = 0
        ss4 = 0
        len_freq = 0
        ...
        if int(x['PMID']) <= 27868941:
                ...
                eml4 = [True for x in wordfreq.items() if 'eml4' in x]
                alk = [True for x in wordfreq.items() if 'alk' in x]
                # ---------------摘要统计-------------------#
                for key in wordfreq:
                ...
                if len(eml4) != 0 and eml4[0] and len(alk) != 0 and alk[0]:
                    for key in wordfreq:
                        if 'eml4' in key:
                            eml4alk_score = eml4alk_score + wordfreq[key]
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'transcript' in key1:
                        transcript_score = transcript_score + wordfreq[key]
                ...
                if len(lung) != 0 and lung[0]:
                        for ele in ChemicalNameList:
                            if 'EML4-ALK' in ele['NameOfSubstance']:
                                gx = idf_eml4alk
                                break
                ...
                if len(lung) != 0 and lung[0]:
                        for eleM in MeshHeadingNameList:
                            if 'EML4-ALK' in eleM['MeshHeadingName']:
                                gx = idf_eml4alk
                                break
                ...
                bm25_eml4alk_score = (((k1 + 1) * eml4alk_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + eml4alk_score))
                bm25_transcript_score = (((k1 + 1) * transcript_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + transcript_score))

                bm25_ab_score = idf_lung * bm25_lung_score + idf_fusion * bm25_fusion_score + idf_eml4alk * bm25_eml4alk_score+idf_transcript*bm25_transcript_score

                idf_para = [{str(lung_score): idf_lung}, {str(fusion_score): idf_fusion},
                            {str(eml4alk_score): idf_eml4alk},{str(transcript_score): idf_transcript}]
                for ele in ChemicalNameList:
                    if re.findall(r'(EML4-ALK)', ele['NameOfSubstance']):
                        ss1 = ss1 + idf_ele_1
                        break
                for ele in ChemicalNameList:
                ...
                for eleK in KeywordsList:
                    if 'lung' in str(eleK).lower():
                        ...
                        break
                for eleK in KeywordsList:
                    if re.findall(r'(eml4-alk)', str(eleK).lower()):
                        ss4 = ss4 + idf_eleK_2
                        break
