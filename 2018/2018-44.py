...
mytopicdb=myclient["cs2018_44"]
mydata=mytopicdb["cs2018_score_44"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_44_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    b1=0.75
    b2=0.75
    idf_glioma = log((29138919 - 37608 + 0.5) / (37608 + 0.5), 10)
    ...
    idf_eleM_1 = log((25389659 - 35956 + 0.5) / (35956 + 0.5), 10)
    ...
    idf_eleK_1 = log((5435471 - 9233 + 0.5) / (9233 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                wordfreq = x['wordfreq']
                glioma = [True for x in wordfreq.items() if 'glioma' in x]
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'glioma' in key1:
                        glioma_score = glioma_score + wordfreq[key]
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'braf' == key1:
                ...
                bm25_glioma_score = (((k1+1)*glioma_score)/((k1*(b1+(1-b1)*(len_freq/85)))+glioma_score))
                bm25_braf_score = (((k1 + 1) * braf_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + braf_score))


                bm25_ab_score =idf_glioma*bm25_glioma_score+idf_braf*bm25_braf_score

                idf_para=[{str(glioma_score):idf_glioma},{str(braf_score):idf_braf}]

                # ---------------共现分析摘要-------------------#
                if len(glioma) != 0 and glioma[0]:
                    for key in wordfreq:
                        key = cop.sub('', key)
                        if 'braf' == key:
                            gx = idf_braf
