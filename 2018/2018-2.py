...
mytopicdb=myclient["cs2018_2"]
mydata=mytopicdb["cs2018_score_2"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_2_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                bm25_v600k_score = (((k1+1)*v600k_score)/((k1*(b1+(1-b1)*(len_freq/85)))+v600k_score))
                bm25_ab_score =idf_melanoma*bm25_melanoma_score+idf_braf*bm25_braf_score+idf_v600k*bm25_v600k_score

                idf_para=[{str(melanoma_score):idf_melanoma},{str(braf_score):idf_braf},{str(v600k_score):idf_v600k}]
            ...
            # ---------------共现分析化学-------------------#
                if len(melanoma) != 0 and melanoma[0]:
                    for ele in ChemicalNameList:
                        if 'V600K' in ele['NameOfSubstance']:
                            gx1 = idf_v600k
                            break
                if len(melanoma) != 0 and melanoma[0]:
                    for ele in ChemicalNameList:
             ...
             # ---------------共现分析医学主题词-------------------#
                if len(melanoma) != 0 and melanoma[0]:
                    for eleM in MeshHeadingNameList:
                        if 'V600K' in eleM['MeshHeadingName']:
                            gx1 = idf_v600k
                            break
                if len(melanoma) != 0 and melanoma[0]:
                    for eleM in MeshHeadingNameList:
                ...
                for ele in ChemicalNameList:
                    if 'Skin Neoplasms' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_1
                        break
                for ele in ChemicalNameList:
