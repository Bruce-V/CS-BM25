...
mytopicdb=myclient["cs2018_27"]
mydata=mytopicdb["cs2018_score_27"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_27_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_eleM_8 = log((25389659 - 8363 + 0.5) / (8363 + 0.5), 10)
    idf_eleM_9 = log((25389659 - 4785026 + 0.5) / (4785026 + 0.5), 10)

    idf_eleK_1 = log((5435471 - 233 + 0.5) / (233 + 0.5), 10)
    idf_eleK_2 = log((5435471 - 147881 + 0.5) / (147881 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                wordfreq = x['wordfreq']
                medullary = [True for x in wordfreq.items() if 'medullary' in x]
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'medullary' in key1:
                        medullary_score = medullary_score + wordfreq[key]
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'thyroid' in key1:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'ret' == key1:
                        ret_score = ret_score + wordfreq[key]


                bm25_medullary_score = (((k1+1)*medullary_score)/((k1*(b1+(1-b1)*(len_freq/85)))+medullary_score))
                ...
                bm25_ret_score = (((k1 + 1) * ret_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + ret_score))


                bm25_ab_score =idf_medullary*bm25_medullary_score+idf_thyroid*bm25_thyroid_score+idf_carcinoma*bm25_carcinoma_score+idf_ret*bm25_ret_score

                idf_para=[{str(medullary_score):idf_medullary},{str(thyroid_score):idf_thyroid},{str(carcinoma_score):idf_carcinoma},{str(ret_score):idf_ret}]

                # ---------------共现分析摘要-------------------#
                if len(medullary) != 0 and medullary[0] and len(thyroid) != 0 and thyroid[0] and len(carcinoma) != 0 and carcinoma[0]:
                    for key in wordfreq:
                        key = cop.sub('', key)
                        if 'ret' == key:
                            gx = idf_ret
            ...
            # ---------------共现分析化学-------------------#
                if len(medullary) != 0 and medullary[0] and len(thyroid) != 0 and thyroid[0] and len(carcinoma) != 0 and carcinoma[0]:
                    for ele in ChemicalNameList:
                        if 'RET' in ele['NameOfSubstance']:
                            gx = idf_ret
                            break
            # ---------------共现分析关键字-------------------#
                if len(medullary) != 0 and medullary[0] and len(thyroid) != 0 and thyroid[0] and len(carcinoma) != 0 and carcinoma[0]:
                    for eleK in KeywordsList:
                        if 'ret' == str(eleK).lower():
                            gx = idf_ret
                            break
             # ---------------共现分析医学主题词-------------------#
                if len(medullary) != 0 and medullary[0] and len(thyroid) != 0 and thyroid[0] and len(carcinoma) != 0 and carcinoma[0]:
                    for eleM in MeshHeadingNameList:
                        if 'RET' in eleM['MeshHeadingName']:
                            gx = idf_ret
                            break

                for ele in ChemicalNameList:
                    if 'Thyroid cancer, medullary' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_1
                        break
                for ele in ChemicalNameList:
                    if 'Carcinoma, Neuroendocrine' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_2
                        break
                for ele in ChemicalNameList:
                ...
                for eleM in MeshHeadingNameList:
                    if 'RET' in eleM['MeshHeadingName']:
                        ...
                        break
                for eleM in MeshHeadingNameList:
                    if 'Thyroid cancer, medullary' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_2
                        break
                for eleM in MeshHeadingNameList:
                    if 'Carcinoma, Neuroendocrine' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_3
                        break
                for eleM in MeshHeadingNameList:
                ...
                for eleK in KeywordsList:
                    if 'medullary thyroid carcinoma' in str(eleK).lower():
                        ss4 = ss4 + idf_eleK_1
                        break
                for eleK in KeywordsList:
                    if 'ret' in str(eleK).lower():
                        ss4 = ss4 + idf_eleK_2
                        break
