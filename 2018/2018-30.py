...
mytopicdb=myclient["cs2018_30"]
mydata=mytopicdb["cs2018_score_30"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_30_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_ros1 = log((29138919 - 840 + 0.5) / (840 + 0.5), 10)
    ...
    idf_ele_5 = log((13670358 - 430 + 0.5) / (430 + 0.5), 10)

    idf_eleM_1 = log((25389659 - 200349 + 0.5) / (200349 + 0.5), 10)
    idf_eleM_2 = log((25389659 - 33948 + 0.5) / (33948+ 0.5), 10)
    idf_eleM_3 = log((25389659 - 56642 + 0.5) / (56642 + 0.5), 10)
    ...
    idf_eleK_2 = log((5435471 - 202 + 0.5) / (202 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'ros1' == key1:
                        ros1_score = ros1_score + wordfreq[key]



                bm25_lung_score = (((k1+1)*lung_score)/((k1*(b1+(1-b1)*(len_freq/85)))+lung_score))
                bm25_ros1_score = (((k1 + 1) * ros1_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + ros1_score))



                bm25_ab_score =idf_lung*bm25_lung_score+idf_ros1*bm25_ros1_score

                idf_para=[{str(lung_score):idf_lung},{str(ros1_score):idf_ros1}]
            ...
            # ---------------共现分析化学-------------------#
                if len(lung) != 0 and lung[0]:
                    for ele in ChemicalNameList:
                        if 'ROS1' in ele['NameOfSubstance']:
                            gx = idf_ros1
                            break
            # ---------------共现分析关键字-------------------#
                if len(lung) != 0 and lung[0]:
                    for eleK in KeywordsList:
                        if 'ros1' in str(eleK).lower():
                            gx = idf_ros1
                            break
             # ---------------共现分析医学主题词-------------------#
                if len(lung) != 0 and lung[0]:
                    for eleM in MeshHeadingNameList:
                        if 'ROS1' in eleM['MeshHeadingName']:
                            gx = idf_ros1
                            break
