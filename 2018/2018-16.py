...
mytopicdb=myclient["cs2018_16"]
mydata=mytopicdb["cs2018_score_16"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_16_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_rearrangement = log((29138919 - 77805 + 0.5) / (77805 + 0.5), 10)
    ...
    idf_eleM_1 = log((25389659 - 18028 + 0.5) / (18028 + 0.5), 10)
    idf_eleM_2 = log((25389659 - 10626 + 0.5) / (10626 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'rearrangement' in key1:
                        rearrangement_score = rearrangement_score + wordfreq[key]



                bm25_melanoma_score = (((k1+1)*melanoma_score)/((k1*(b1+(1-b1)*(len_freq/85)))+melanoma_score))
                bm25_ntrk1_score =(((k1+1)*ntrk1_score)/((k1*(b1+(1-b1)*(len_freq/85)))+ntrk1_score))
                bm25_rearrangement_score = (((k1+1)*rearrangement_score)/((k1*(b1+(1-b1)*(len_freq/85)))+rearrangement_score))


                bm25_ab_score =idf_melanoma*bm25_melanoma_score+idf_ntrk1*bm25_ntrk1_score+idf_rearrangement*bm25_rearrangement_score

                idf_para=[{str(melanoma_score):idf_melanoma},{str(ntrk1_score):idf_ntrk1},{str(rearrangement_score):idf_rearrangement}]
                ...
                if len(melanoma)!=0 and melanoma[0]:
                    for key in wordfreq:
                        ...
                        if 'ntrk1' in key:
                            gx = idf_ntrk1
                        if 'rearrangement' in key:
                            gx1 = idf_rearrangement
            ...
            # ---------------共现分析关键字-------------------#
                ...
                if len(melanoma) != 0 and melanoma[0]:
                    for eleK in KeywordsList:
                        if 'rearrangement' in str(eleK).lower():
                            gx1= idf_rearrangement
                            break
             ...
             # ---------------共现分析医学主题词-------------------#
                ...
                for ele in ChemicalNameList:
                    if 'Carcinoma, Papillary' == ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_1
                        break
                for ele in ChemicalNameList:
                    if 'Gene Rearrangement' == ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_2
                        break
                ...
                for eleM in MeshHeadingNameList:
                    if 'Carcinoma, Papillary' == eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_1
                        break
                for eleM in MeshHeadingNameList:
                    if 'Gene Rearrangement' == eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_2
                        break
                for eleM in MeshHeadingNameList:
