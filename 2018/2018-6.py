...
mytopicdb=myclient["cs2018_6"]
mydata=mytopicdb["cs2018_score_6"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_6_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_q61r = log((29138919 - 112 + 0.5) / (112 + 0.5), 10)
    ...
    idf_eleK_3 = log((5435471 - 4 + 0.5) / (4 + 0.5), 10)
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'q61r' in key1:
                        q61r_score = q61r_score + wordfreq[key]
                ...
                bm25_q61r_score = (((k1 + 1) * q61r_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + q61r_score))

                bm25_ab_score =idf_melanoma*bm25_melanoma_score+idf_braf*bm25_braf_score+idf_v600e*bm25_v600e_score+idf_nras*bm25_nras_score+idf_q61r*bm25_q61r_score

                idf_para=[{str(melanoma_score):idf_melanoma},{str(braf_score):idf_braf},{str(v600e_score):idf_v600e},{str(nras_score):idf_nras},{str(q61r_score):idf_q61r}]
            ...
            # ---------------共现分析化学-------------------#
                ...
                if len(melanoma) != 0 and melanoma[0]:
                    for ele in ChemicalNameList:
                        if 'Q61R' in ele['NameOfSubstance']:
                            gx3 = idf_q61r
                            break
            # ---------------共现分析关键字-------------------#
                if len(melanoma) != 0 and melanoma[0]:
             ...
             # ---------------共现分析医学主题词-------------------#
                ...
                if len(melanoma) != 0 and melanoma[0]:
                    for eleM in MeshHeadingNameList:
                        if 'Q61R' in eleM['MeshHeadingName']:
                            gx3 = idf_q61r
                            break
