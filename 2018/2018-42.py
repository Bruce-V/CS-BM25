...
mytopicdb=myclient["cs2018_42"]
mydata=mytopicdb["cs2018_score_42"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_42_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_cdk6 = log((29138919 - 1191 + 0.5) / (1191 + 0.5), 10)
    ...
    idf_ele_4 = log((13670358 - 596 + 0.5) / (596 + 0.5), 10)
    idf_ele_5 = log((13670358 - 1056 + 0.5) / (1056 + 0.5), 10)
    ...
    idf_eleM_7 = log((25389659 - 823510 + 0.5) / (823510 + 0.5), 10)
    idf_eleM_8 = log((25389659 - 1056 + 0.5) / (1056 + 0.5), 10)
    ...
    idf_eleK_2 = log((5435471 - 89 + 0.5) / (89 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'glioblastoma' in key1:
                        glioblastoma_score = glioblastoma_score + wordfreq[key]
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'cdk6' == key1:
                        cdk6_score = cdk6_score + wordfreq[key]




                bm25_glioblastoma_score = (((k1+1)*glioblastoma_score)/((k1*(b1+(1-b1)*(len_freq/85)))+glioblastoma_score))
                bm25_cdk6_score = (((k1 + 1) * cdk6_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + cdk6_score))


                bm25_ab_score =idf_glioblastoma*bm25_glioblastoma_score+idf_cdk6*bm25_cdk6_score

                idf_para=[{str(glioblastoma_score):idf_glioblastoma},{str(cdk6_score):idf_cdk6}]
            ...
            # ---------------共现分析化学-------------------#
                if len(glioblastoma) != 0 and glioblastoma[0]:
                    for ele in ChemicalNameList:
                        if 'CDK6' in ele['NameOfSubstance']:
                            gx = idf_cdk6
                            break
             ...
             # ---------------共现分析医学主题词-------------------#
                if len(glioblastoma) != 0 and glioblastoma[0]:
                    for eleM in MeshHeadingNameList:
                        if 'CDK6' in eleM['MeshHeadingName']:
                            gx = idf_cdk6
                            break


                for ele in ChemicalNameList:
                    if 'Glioblastoma' == ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_1
                        break
                for ele in ChemicalNameList:
                    if 'Brain Neoplasms' == ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_2
                        break
                for ele in ChemicalNameList:
                    if 'Cell Line, Tumor' == ele['NameOfSubstance']:
                        ...
                        break
                for ele in ChemicalNameList:
                    if 'CDK6 protein, human' == ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_4
                        break
                for ele in ChemicalNameList:
                    if 'Cyclin-Dependent Kinase 6' == ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_5
                        break


                for eleM in MeshHeadingNameList:
                    if 'Glioblastoma' == eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_1
                        break
                for eleM in MeshHeadingNameList:
                    if 'Brain Neoplasms' == eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_2
                        break
                for eleM in MeshHeadingNameList:
                    if 'Cell Line, Tumor' == eleM['MeshHeadingName']:
                        ...
                        break
                for eleM in MeshHeadingNameList:
                    if 'CDK6 protein, human' == eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_4
                        break
                ...
                for eleM in MeshHeadingNameList:
                    if 'Aged, 80 and over' == eleM['MeshHeadingName']:
                        ...
                        break
                for eleM in MeshHeadingNameList:
                    if 'Cyclin-Dependent Kinase 6' == eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_8
                        break
