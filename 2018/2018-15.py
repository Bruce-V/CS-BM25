...
mytopicdb=myclient["cs2018_15"]
mydata=mytopicdb["cs2018_score_15"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_15_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_nf1 = log((29138919 - 12558 + 0.5) / (12558 + 0.5), 10)
    idf_truncation = log((29138919 - 15080 + 0.5) / (15080 + 0.5), 10)




    idf_ele_1 = log((13670358 - 1158 + 0.5) / (1158 + 0.5), 10)
    idf_ele_2 = log((13670358 - 0 + 0.5) / (0 + 0.5), 10)
    idf_ele_3 = log((13670358 - 0 + 0.5) / (0 + 0.5), 10)
    idf_ele_4 = log((13670358 - 0 + 0.5) / (0 + 0.5), 10)

    idf_eleM_1 = log((25389659 - 1158 + 0.5) / (1158 + 0.5), 10)
    idf_eleM_2 = log((25389659 - 9269 + 0.5) / (9269 + 0.5), 10)
    ...
    idf_eleK_2 = log((5435471 - 686 + 0.5) / (686 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'nf1' in key1:
                        nf1_score = nf1_score + wordfreq[key]
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'truncation' in key1:
                        truncation_score = truncation_score + wordfreq[key]



                bm25_melanoma_score = (((k1+1)*melanoma_score)/((k1*(b1+(1-b1)*(len_freq/85)))+melanoma_score))
                bm25_nf1_score =(((k1+1)*nf1_score)/((k1*(b1+(1-b1)*(len_freq/85)))+nf1_score))
                bm25_truncation_score = (((k1+1)*truncation_score)/((k1*(b1+(1-b1)*(len_freq/85)))+truncation_score))


                bm25_ab_score =idf_melanoma*bm25_melanoma_score+idf_nf1*bm25_nf1_score+idf_truncation*bm25_truncation_score

                idf_para=[{str(melanoma_score):idf_melanoma},{str(nf1_score):idf_nf1},{str(truncation_score):idf_truncation}]
            ...
            # ---------------共现分析关键字-------------------#
                ...
                if len(melanoma) != 0 and melanoma[0]:
                    for eleK in KeywordsList:
                        if 'nf1' in str(eleK).lower():
                            gx = idf_nf1
                            break
                if len(melanoma) != 0 and melanoma[0]:
                    for eleK in KeywordsList:
                        if 'truncation' in str(eleK).lower():
                            gx1= idf_truncation
                            break
             ...
             # ---------------共现分析医学主题词-------------------#
                ...
                if len(melanoma) != 0 and melanoma[0]:
                    for eleM in MeshHeadingNameList:
                        if 'NF1' in eleM['MeshHeadingName']:
                            gx = idf_nf1
                            break


                for ele in ChemicalNameList:
                    if 'Neurofibromin 1' == ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_1
                        break
                for ele in ChemicalNameList:
                    if 'Neurofibromatosis 1' == ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_2
                        break
                ...
                for eleM in MeshHeadingNameList:
                    if 'Neurofibromin 1' == eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_1
                        break
                for eleM in MeshHeadingNameList:
                    if 'Neurofibromatosis 1' == eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_2
                        break
                for eleM in MeshHeadingNameList:
