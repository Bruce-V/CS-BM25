...
mytopicdb=myclient["cs2018_4"]
mydata=mytopicdb["cs2018_score_4"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_4_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_k601e = log((29138919 - 54 + 0.5) / (54 + 0.5), 10)

    idf_ele_1 = log((13670358 - 0 + 0.5) / (1130 + 0.5), 10)
    idf_ele_2 = log((13670358 - 7222 + 0.5) / (5676 + 0.5), 10)
    idf_ele_3 = log((13670358 - 0 + 0.5) / (0 + 0.5), 10)

    idf_eleM_1 = log((25389659 - 56983 + 0.5) / (56983 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'k601e' in key1:
                        k601e_score = k601e_score + wordfreq[key]
                ...
                bm25_k601e_score = (((k1+1)*k601e_score)/((k1*(b1+(1-b1)*(len_freq/85)))+k601e_score))
                bm25_ab_score =idf_melanoma*bm25_melanoma_score+idf_braf*bm25_braf_score+idf_k601e*bm25_k601e_score

                idf_para=[{str(melanoma_score):idf_melanoma},{str(braf_score):idf_braf},{str(k601e_score):idf_k601e}]
            ...
            # ---------------共现分析化学-------------------#
                if len(melanoma) != 0 and melanoma[0]:
                    for ele in ChemicalNameList:
                        if 'K601E' in ele['NameOfSubstance']:
                            gx1 = idf_k601e
                            break
                if len(melanoma) != 0 and melanoma[0]:
                    for ele in ChemicalNameList:
             ...
             # ---------------共现分析医学主题词-------------------#
                if len(melanoma) != 0 and melanoma[0]:
                    for eleM in MeshHeadingNameList:
                        if 'K601E' in eleM['MeshHeadingName']:
                            gx1 = idf_k601e
                            break
                if len(melanoma) != 0 and melanoma[0]:
                    for eleM in MeshHeadingNameList:
                ...
                for ele in ChemicalNameList:
                    if 'DNA Mutational Analysis' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_1
                        break
                for ele in ChemicalNameList:
                ...
                for eleM in MeshHeadingNameList:
                    if 'DNA Mutational Analysis' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_1
                        break
                for eleM in MeshHeadingNameList:
