...
mytopicdb=myclient["cs2018_13"]
mydata=mytopicdb["cs2018_score_13"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_13_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_n822y = log((29138919 - 4 + 0.5) / (4 + 0.5), 10)
    ...
    idf_eleM_2 = log((25389659 - 5700 + 0.5) / (5700 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'n822y' in key1:
                        n822y_score = n822y_score + wordfreq[key]
                ...
                bm25_n822y_score = (((k1+1)*n822y_score)/((k1*(b1+(1-b1)*(len_freq/85)))+n822y_score))


                bm25_ab_score =idf_melanoma*bm25_melanoma_score+idf_kit*bm25_kit_score+idf_n822y*bm25_n822y_score

                idf_para=[{str(melanoma_score):idf_melanoma},{str(kit_score):idf_kit},{str(n822y_score):idf_n822y}]
            ...
            # ---------------共现分析化学-------------------#
                ...
                if len(melanoma) != 0 and melanoma[0]:
                    for ele in ChemicalNameList:
                        if 'N822Y' in ele['NameOfSubstance']:
                            gx1 = idf_n822y
                            break
             ...
             # ---------------共现分析医学主题词-------------------#
                ...
                if len(melanoma) != 0 and melanoma[0]:
                    for eleM in MeshHeadingNameList:
                        if 'N822Y' in eleM['MeshHeadingName']:
                            gx = idf_n822y
                            break
