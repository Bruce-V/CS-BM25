...
mytopicdb=myclient["cs2018_21"]
mydata=mytopicdb["cs2018_score_21"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_21_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_extensive = log((29138919 - 393844 + 0.5) / (393844 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'extensive' in key1:
                        extensive_score = extensive_score + wordfreq[key]
                for key in wordfreq:
                    key1 = cop.sub('', key)
                ...
                bm25_extensive_score = (((k1+1)*extensive_score)/((k1*(b1+(1-b1)*(len_freq/85)))+extensive_score))
                ...
                bm25_ab_score =idf_melanoma*bm25_melanoma_score+idf_tumor*bm25_tumor_score+idf_extensive*bm25_extensive_score+idf_infiltrating*bm25_infiltrating_score+idf_lymphocyte*bm25_lymphocyte_score

                idf_para=[{str(melanoma_score):idf_melanoma},{str(tumor_score):idf_tumor},{str(extensive_score):idf_extensive},{str(infiltrating_score):idf_infiltrating},{str(lymphocyte_score):idf_lymphocyte}]
