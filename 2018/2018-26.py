...
mytopicdb=myclient["cs2018_26"]
mydata=mytopicdb["cs2018_score_26"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_26_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    b1=0.75
    b2=0.75
    idf_colorectal = log((29138919 - 129693 + 0.5) / (129693 + 0.5), 10)
    ...
    idf_ele_4 = log((13670358 - 80567 + 0.5) / (80567 + 0.5), 10)
    idf_ele_5 = log((13670358 - 629 + 0.5) / (629 + 0.5), 10)

    idf_eleM_1 = log((25389659 - 88975 + 0.5) / (88975 + 0.5), 10)
    idf_eleM_2 = log((25389659 - 80617 + 0.5) / (80617 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                bm25_colorectal_score = (((k1+1)*colorectal_score)/((k1*(b1+(1-b1)*(len_freq/85)))+colorectal_score))
                bm25_nras_score = (((k1 + 1) * nras_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + nras_score))


                bm25_ab_score =idf_colorectal*bm25_colorectal_score+idf_nras*bm25_nras_score

                idf_para=[{str(colorectal_score):idf_colorectal},{str(nras_score):idf_nras}]
            ...
            # ---------------共现分析关键字-------------------#
             ...
             # ---------------共现分析医学主题词-------------------#
                ...
                for ele in ChemicalNameList:
                    if 'Colorectal Neoplasms' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_1
                        break
                for ele in ChemicalNameList:
