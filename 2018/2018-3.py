...
mytopicdb=myclient["cs2018_3"]
mydata=mytopicdb["cs2018_score_3"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_3_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_v600r = log((29138919 - 40 + 0.5) / (40 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'v600r' in key1:
                        v600r_score = v600r_score + wordfreq[key]
                ...
                bm25_v600r_score = (((k1+1)*v600r_score)/((k1*(b1+(1-b1)*(len_freq/85)))+v600r_score))
                bm25_ab_score =idf_melanoma*bm25_melanoma_score+idf_braf*bm25_braf_score+idf_v600r*bm25_v600r_score

                idf_para=[{str(melanoma_score):idf_melanoma},{str(braf_score):idf_braf},{str(v600r_score):idf_v600r}]
            ...
            # ---------------共现分析化学-------------------#
                if len(melanoma) != 0 and melanoma[0]:
                    for ele in ChemicalNameList:
                        if 'V600R' in ele['NameOfSubstance']:
                            gx1 = idf_v600r
                            break
                if len(melanoma) != 0 and melanoma[0]:
                    for ele in ChemicalNameList:
             ...
             # ---------------共现分析医学主题词-------------------#
                if len(melanoma) != 0 and melanoma[0]:
                    for eleM in MeshHeadingNameList:
                        if 'V600R' in eleM['MeshHeadingName']:
                            gx1 = idf_v600r
                            break
                if len(melanoma) != 0 and melanoma[0]:
                    for eleM in MeshHeadingNameList:
