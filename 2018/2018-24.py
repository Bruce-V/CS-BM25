...
mytopicdb=myclient["cs2018_24"]
mydata=mytopicdb["cs2018_score_24"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2018_score_24_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_apc = log((29138919 - 20305 + 0.5) / (20305 + 0.5), 10)
    ...
    idf_ele_2 = log((13670358 - 1856 + 0.5) / (1856 + 0.5), 10)
    idf_ele_3 = log((13670358 - 0 + 0.5) / (0 + 0.5), 10)

    idf_eleM_1 = log((25389659 - 88975 + 0.5) / (88975 + 0.5), 10)
    idf_eleM_2 = log((25389659 - 2531 + 0.5) / (2531 + 0.5), 10)
    idf_eleM_3 = log((25389659 - 113103 + 0.5) / (113103 + 0.5), 10)
    idf_eleM_4 = log((25389659 - 73839 + 0.5) / (73839 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        if int(x['PMID']) <= 27868941:
                ...
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'apc' == key1:
                        apc_score = apc_score + wordfreq[key]
                for key in wordfreq:
                    key1 = cop.sub('', key)
                    if 'loss' in key1:
                ...
                bm25_apc_score = (((k1 + 1) * apc_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + apc_score))
                ...
                bm25_ab_score =idf_melanoma*bm25_melanoma_score+idf_apc*bm25_apc_score+idf_loss*bm25_loss_score+idf_function*bm25_function_score

                idf_para=[{str(melanoma_score):idf_melanoma},{str(apc_score):idf_apc},{str(loss_score):idf_loss},{str(function_score):idf_function}]
            ...
            # ---------------共现分析化学-------------------#
                if len(melanoma) != 0 and melanoma[0]:
                    for ele in ChemicalNameList:
                        if 'Genes, APC' in ele['NameOfSubstance']:
                            gx2 = idf_apc
                            break
            # ---------------共现分析关键字-------------------#
                if len(melanoma) != 0 and melanoma[0]:
                    for eleK in KeywordsList:
                        if 'apc' in str(eleK).lower():
                            gx2 = idf_apc
                            break
             # ---------------共现分析医学主题词-------------------#
                if len(melanoma) != 0 and melanoma[0]:
                    for eleM in MeshHeadingNameList:
                        if 'Genes, APC' in eleM['MeshHeadingName']:
                            gx2 = idf_apc
                            break
                ...
                for ele in ChemicalNameList:
                    if 'Genes, APC' in ele['NameOfSubstance']:
                        ...
                        break
                for ele in ChemicalNameList:
                    if 'Adenoma' in ele['NameOfSubstance']:
                        ss1 = ss1 + idf_ele_2
                        break
                ...
                for eleM in MeshHeadingNameList:
                    if 'Skin Neoplasms' in eleM['MeshHeadingName']:
                        ...
                        break
                for eleM in MeshHeadingNameList:
                    if 'Adenoma' in eleM['MeshHeadingName']:
                        ss2 = ss2 + idf_eleM_4
                        break
                for eleM in MeshHeadingNameList:
